...
- (void)updateProgressCircle:(NSTimer *)timer{
  ...
  //  NSLog(@"the Duration time_______________%f",self.player.duration);
    ...
    //  self.progress = (float) (self.player.currentTime / 60.0);
...
- (void)play{
    if (!self.player.playing) {
        ...
        self.timer = [NSTimer scheduledTimerWithTimeInterval:0.001f target:self selector:@selector(updateProgressCircle:) userInfo:nil repeats:YES];
        [self.player play];
...
-(void)setWarningShadow:(int)num gameMode:(BOOL)gameView{
    
    if (gameView==YES) {
        if (num>30)
        {
            _progressLayer.shadowOpacity = 1.0;
            _progressLayer.shadowColor = [UIColor colorWithRed:201.0 / 255.0
                                                         green:43.0 / 255.0
                                                          blue:37.0 / 255.0
                                                         alpha:1.0].CGColor;
        }
        ...
        {
            _progressLayer.shadowOpacity = 0.0;
            _progressLayer.shadowColor = [UIColor clearColor].CGColor;
        }
    }
    else{
        if (num>30)
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)recognizer{
    if (recognizer.state == UIGestureRecognizerStateChanged) {
        ...
        else self.timer = [NSTimer scheduledTimerWithTimeInterval:0.001f target:self selector:@selector(updateProgressCircle:) userInfo:nil repeats:YES];
        [self.delegate updatePlayOrPauseButton];
    }
}
